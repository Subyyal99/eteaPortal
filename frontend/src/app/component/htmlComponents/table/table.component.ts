...
export class TableComponent implements OnInit, OnChanges {
  ...
  updatePaginatorValues(event) {
    this.updatePaginator.emit(event);
  }
  updateData(value, type) {
    this.inputInfo[type] = value;
    let obj = { value: value, type: type };
    this.selectedValueChange.emit(obj);
  }
  deleteConfirmationVisible = false;
  @Input() deleteConfirmationMessage: any =
  ...
  executeFunction(functionName: string, rowData: any, action: any) {
    if (action.modalMessage) {
      this.deleteConfirmationMessage = action.modalMessage;
    }
    if (functionName) {
      if (functionName == "deleteRow") {
        ...
        this.rowActionClicked.emit({ functionName, rowData });
      }
    } else {
    }
  }
  ...
  //   { id: 4, date: new Date() },
  //   { id: 5, name: "Bobi", age: 40, date: new Date(), salary: 10000 },
  // ];

  // myColumns = [
  //   { header: "ID", field: "id", type: "text" },
  //   { header: "Name", field: "name", type: "text" },
  //   { header: "Age", field: "age", type: "text" },
  //   { header: "Date", field: "date", type: "date" },
  //   { header: "Salary", field: "salary", type: "currency" },
  // ];

  // onRowActionClicked(event: { functionName: string; rowData: any }) {
  //   const { functionName, rowData } = event;
  //   if (functionName === "deleteRow") {
  //     this.onDeleteClicked(rowData);
  //   } else if (functionName === "editRow") {
  //     this.onEditClicked(rowData);
  //   } else if (functionName === "viewRow") {
  //     this.onViewClicked(rowData);
  //   } else {
  //     this.anyfunc(rowData);
  //   }
  // }
  ...
  dateFilter(date) {
    if (date[0] && date[1]) {
      this.inputInfo[0].check_value = date;
      let dateFrom =
  ...
  shortCodeFilter(data) {
    if (!data) {
      this.col = this.originalData;
      this.inputInfo2[0].val = "";
      delete this.inputInfo2[0].check_value;
      this.designFlag2 = false;
    } else {
      this.inputInfo2[0].val = data;
      this.inputInfo2[0].check_value = data;
      this.tableData = this.col;
      this.col = [];
